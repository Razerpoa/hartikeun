...
## Project
Express + React SPA for Indonesian dialect, slang, Brainrot, and foreign language translation via Google Gemini. A "Polyglot Indonesian Bridge" that auto-detects any input and translates TO Indonesian with cultural insights.
...
├── types.ts              # TransformationResult with learning_insight, language_variety
...
│   ├── utils.ts          # normalizeSlang() with ABBREVIATION_MAP, getCacheKey()
...
│   └── routes/
│       ├── transform.ts    # POST /api/transform (polyglot prompt + responseSchema)
│       ├── wordDetails.ts  # POST /api/word-details
│       └── tts.ts          # POST /api/tts
├── components/             # React: LandingPage, Translator, WordDetails
│   └── ui/                 # Extracted UI components (WordToken, CopyButton, ChatMessageBubble)
│       └── DiscoveryPanel  # Learning insight panel (in Translator)
```
...
### `normalizeSlang()` expands Indonesian text-speak
`utils.ts` has an `ABBREVIATION_MAP` that expands common abbreviations before vowel/consonant normalization: `yg`→`yang`, `ga/gk/g`→`tidak`, `bgt`→`banget`, `gw/gue`→`saya`, etc. This runs on every input before reaching the AI.
...
- `POST /api/transform` — text + optional base64 image, max 5000 chars, max 5MB image. Uses a **polyglot prompt**: auto-detects source language (Indonesian dialect, Gen-Z slang, Brainrot, or foreign language) and translates TO Indonesian + English. Returns `learning_insight` and `language_variety` fields.
...
- 46 tests total (19 unit + 16 integration + 11 frontend/validation)
